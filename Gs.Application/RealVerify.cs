...
namespace Gs.Application
{
    ...
    /// 支付实名验证
    /// </summary>
    public class RealVerify : IRealVerify
    {
        private readonly HttpClient client;
        private readonly Models.RealVerifyConfig config;
        public RealVerify(IHttpClientFactory factory, IOptionsMonitor<Models.RealVerifyConfig> monitor)
        {
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        public RspRealVerifyInitiate ExecuteNew(string OuterOrderNo, string CertName, string CertNo, string MetaInfo, string UserId)
        {
            RspRealVerifyInitiate result = new RspRealVerifyInitiate();
            //NEWSDK
            IClientProfile profile = DefaultProfile.GetProfile("cn-hangzhou", config.AccessKey, config.AccessSecret);
            DefaultAcsClient client = new DefaultAcsClient(profile);

            var request = new InitFaceVerifyRequest();
            request.OuterOrderNo = OuterOrderNo;
            request.ProductCode = "ID_PRO";
            request.CertType = "IDENTITY_CARD";
            request.CertName = CertName;
            request.CertNo = CertNo;
            request.MetaInfo = MetaInfo;//"{\"zimVer\":\"3.0.0\",\"appVersion\": \"1\",\"bioMetaInfo\": \"4.1.0:11501568,0\",\"appName\": \"com.aliyun.antcloudauth\",\"deviceType\": \"ios\",\"osVersion\": \"iOS 10.3.2\",\"apdidToken\": \"\",\"deviceModel\": \"iPhone9,1\"}";
            request.UserId = UserId;
            request.SceneId = 1000001375;
            try
            {
                var response = client.GetAcsResponse(request);
                result.ErrCode = response.Code;
                result.ErrMsg = response.Message;
                result.Data.CertifyId = response.ResultObject.CertifyId;
                result.Data.CertifyUrl = response.ResultObject.CertifyId;
                return result;
            }
        ...
        /// 查询认证结果
        /// </summary>
        /// <param name="OuterOrderNo"></param>
        /// <param name="CertName"></param>
        /// <param name="CertNo"></param>
        /// <param name="MetaInfo"></param>
        /// <param name="UserId"></param>
        /// <returns></returns>
        public RspRealVerifyInitiate DescribeFaceVerify(long SceneId, string CertifyId)
        {
            RspRealVerifyInitiate result = new RspRealVerifyInitiate();
            //NEWSDK
            IClientProfile profile = DefaultProfile.GetProfile("cn-hangzhou", config.AccessKey, config.AccessSecret);
            ...
            var request = new DescribeFaceVerifyRequest();
            request.CertifyId = CertifyId;
