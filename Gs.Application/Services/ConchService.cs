...
namespace Gs.Application.Services
{
    ...
    /// NW==贝壳--币
    /// </summary>
    public class ConchService : IConchService
    {
        private readonly String CacheLockKey = "ConchAccount:";
        private readonly String AccountTableName = "user_account_cotton_coin";
        private readonly String RecordTableName = "user_account_cotton_coin_record";

        private readonly WwgsContext context;
        private readonly CSRedisClient RedisCache;
        public ConchService(WwgsContext mysql, CSRedisClient redisClient)
        {
            context = mysql;
        ...
        public async Task<MyResult<List<AccountRecord>>> Records(QueryModel query)
        {
            ...
            {
                ConchModifyType ModifyType = (ConchModifyType)item.ModifyType;
        ...
        /// <returns></returns>
        public async Task<Boolean> ChangeAmount(Int64 Uid, Decimal Amount, ConchModifyType ModifyType, Boolean UseFrozen, params string[] Desc)
        {
            ...
            try
            {
                CacheLock = RedisCache.Lock($"{CacheLockKey}INTEGRAL_{Uid}", 30);
                if (CacheLock == null) { return false; }
        ...
        /// <returns></returns>
        public async Task<MyResult<object>> TodayOutNum(int userId)
        {
            ...
            //今日产出 挖矿
            String SelectRecordSql = $"SELECT IFNULL(Incurred,0) FROM `{RecordTableName}` WHERE `AccountId` = {accInfo.AccountId} And TO_DAYS(`ModifyTime`)=TO_DAYS(now()) and `ModifyType`=5 LIMIT 1";

            String SelectRecordSql2 = $"SELECT IFNULL(Incurred,0) FROM `{RecordTableName}` WHERE `AccountId` = {accInfo.AccountId} And TO_DAYS(`ModifyTime`)=TO_DAYS(now()) and `ModifyType`=7 LIMIT 1";
            var d1 = await context.Dapper.QueryFirstOrDefaultAsync<decimal>(SelectRecordSql);
            var d2 = await context.Dapper.QueryFirstOrDefaultAsync<decimal>(SelectRecordSql2);
            result.Data = $"{Math.Round(d1, 2)}+{Math.Round(d2, 2)}";
            return result;
        }
    }
}
