...
namespace Gs.Application
{
    ...
    {
        private Models.QCloudConfig config;
        private HttpClient client;
        public QCloudPlugin(IHttpClientFactory factory, IOptionsMonitor<Models.QCloudConfig> monitor)
        {
        ...
        public async Task<HttpResponseMessage> Execute<T>(IQCloudRequest<T> request, Int32 minute) where T : QCloudResponse, new()
        {
            ...
            {
                case Enums.QCloudMethod.put:
                    response = await client.PutAsync(Url, request.GetContent());
                    break;
                case Enums.QCloudMethod.get:
                    response = await client.GetAsync(Url);
                    break;
                case Enums.QCloudMethod.post:
                    response = await client.PostAsync(Url, request.GetContent());
                    break;
                case Enums.QCloudMethod.head:
                    break;
                case Enums.QCloudMethod.delete:
                    response = await client.DeleteAsync(Url);
                    break;
                case Enums.QCloudMethod.options:
                    break;
                default:
                    break;
            }
        ...
        /// <param name=""></param>
        /// <returns></returns>
        public async Task<Boolean> PutObject(String path, Stream file)
        {
            StreamContent content = new StreamContent(file);
            HttpResponseMessage response = await Execute(new Request.ReqCosPutObject(path, content), 20);
        ...
        {
            HttpResponseMessage response = await Execute(new Request.ReqCosGetObject(path), 20);
            if (response.StatusCode == HttpStatusCode.OK)
            {
        ...
        {
            HttpResponseMessage response = await Execute(new Request.ReqCosDelObject(path), 20);
        ...
        public async Task<String> PutAuthStr(String path)
        {
            return await Task.Run(() =>
            {
                ...
                return GetAuthStr(new Request.ReqCosPutObject(path, null), 5, HostUrl, this.config.StsSecretId, this.config.StsSecretKey);
            });
        }
        ...
        private static String GetAuthStr<T>(IQCloudRequest<T> request, Int32 minute, String host, String secretId, String secretKey) where T : QCloudResponse, new()
        {
            String HostUrl = host;
            String Path = request.GetPath();
            Enums.QCloudMethod Method = request.GetMethod();
        ...
        /// <returns></returns>
        private static String GetHost(Models.QCloudConfig conf)
        {
